    ...
    function (
        ...
        var setScanQty = function (barcode, imgi2) {
          ...
          SqlService.Select('Imgi2_Picking', '*', strFilter).then(function (results) {
              ...
              if (len > 0) {
                  imgi2.ScanQty = parseInt(results.rows.item(0).ScanQty);
                  if (imgi2.ScanQty !== imgi2.Qty) {
                      imgi2.ScanQty = parseInt(results.rows.item(0).ScanQty) + parseInt($scope.Detail.Scan.Qty);
                      if (imgi2.ScanQty > imgi2.Qty) {
                          var needMoreQty = 0;
                          needMoreQty = parseInt($scope.Detail.Scan.Qty) - (imgi2.ScanQty - imgi2.Qty);
                          PopupService.Alert(popup, 'you scan more ' + (imgi2.ScanQty - imgi2.Qty) + '  just need Scan ' + needMoreQty);
                      } else {
                      ...
                      PopupService.Alert(popup, 'ScanQty  same as Qty');
        ...
        $scope.changeQty = function (type) {
            if (is.not.empty($scope.Detail.Imgi2.BarCode) && hmImgi2.count() > 0) {
                ...
                var promptPopup = $ionicPopup.show({
                    template: '<input type="number" ng-model="Detail.Scan.Qty" ng-change="">',
                    title: 'Enter Qty',
